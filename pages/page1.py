    ...
    with col_left:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Rep1": st.column_config.NumberColumn("Rep1 (抑制率%)", format="%.1f"),
                "Rep2": st.column_config.NumberColumn("Rep2 (抑制率%)", format="%.1f"),
            }
        )
    ...
    with col_right:
        ...
        if not edited_df.empty:
            try:
                ...
                # 2. 计算平均值用于拟合
                mean_inhibition = (rep1 + rep2) / 2.0
                
                # 3. 过滤掉非数值或NaN
                valid_idx = ~np.isnan(conc) & ~np.isnan(mean_inhibition)
                x_data = conc[valid_idx]
                y_data = mean_inhibition[valid_idx]
                ...
                if len(x_data) < 3:
                    ...
                    # 5. 拟合 4PL
                    try:
                        popt, pcov = curve_fit(four_param_logistic, log_x, y_data, 
                                               p0=[100, 1, 0, 0], maxfev=10000)
                        a, b, c, d = popt # Top, Hill, LogIC50, Bottom
                        ic50_val = 10**c
                        fit_success = True
                    except Exception:
                        fit_success = False
                    ...
                    if fit_success:
                        ...
                        ax.set_title(f'IC50 = {ic50_val:.4f}', fontsize=14, color='black', pad=20)
                    else:
                    ...
                    if fit_success:
                        ...
                        href = f'<a href="data:image/svg+xml;base64,{b64}" download="IC50_Curve.svg"><button style="margin-top:10px; padding:5px 10px; background-color:#007bff; color:white; border:none; border-radius:4px; cursor:pointer;">📥 下载矢量图 (SVG)</button></a>'
                    ...
                    if fit_success:
                        ...
                        param_df = pd.DataFrame({
                            "Parameter": ["A (Top)", "B (Hill Slope)", "C (LogIC50)", "D (Bottom)", "IC50"],
                            "Value": [f"{a:.4f}", f"{b:.4f}", f"{c:.4f}", f"{d:.4f}", f"{ic50_val:.4f}"]
                        })
                        st.table(param_df)
...
with tab2:
    ...
    st.info("格式要求：每三列一组（浓度, Rep1, Rep2）。支持 `Compound_A_Conc`, `Compound_A_Inh`, `Unnamed` 格式。")
        ...
        try:
            ...
            for idx in conc_indices:
                ...
                if idx + 1 < len(temp_cols):
                    ...
                    new_name_1 = f"{prefix}_Inh_repeat1"
                    final_col_map[idx+1] = new_name_1
                
                if idx + 2 < len(temp_cols):
                    new_name_2 = f"{prefix}_Inh_repeat2"
            ...
            for col in raw_df.columns:
                if 'Conc' in str(col):
                    ...
                    conc_col = col
                    rep1_col = f"{prefix}_Inh_repeat1"
                    rep2_col = f"{prefix}_Inh_repeat2"
            ...
            if not compounds_data:
                st.warning("未检测到符合格式的数据列（需包含 Conc, Inh_repeat1, Inh_repeat2）。")
            else:
                    ...
                    for j in range(3):
                        ...
                        if index < len(compound_list):
                            ...
                            try:
                                ...
                                ax.set_title(f"{key}\nIC50: {ic50:.3f}", fontsize=10, color='black', pad=10)
                                ax.set_xlabel("Log10(Conc)", fontsize=8)
                                ax.set_ylabel("Inhibition (%)", fontsize=8)
                ...
                if all_charts_data:
                    ...
                    # 写入汇总表头
                    summary_headers = ["化合物名称", "IC50", "A (Top)", "B (Hill)", "C (LogIC50)", "D (Bottom)"]
                    ...
                    for i, (compound_name, data) in enumerate(all_charts_data.items()):
                        ...
                        # 参数值
                        param_keys = ["IC50", "A (Top)", "B (Hill)", "C (LogIC50)", "D (Bottom)"]
                        ...
                        # 3. 在汇总表中添加一行
                        ws_summary.append([params["Compound"], params["IC50"], params["A (Top)"], params["B (Hill)"], params["C (LogIC50)"], params["D (Bottom)"]])
                    ...
                    st.download_button(
                        ...
                        data=output,
                        file_name="IC50_批量分析报告_3x2矩阵.xlsx",
                ...
                if results_summary:
                    ...
                    st.download_button(
                        label="📥 下载结果为 CSV",
                        data=csv,
                        file_name='IC50_Batch_Results.csv',
                        mime='text/csv',
                    )
